  ...
  const fetchPage = async (p = 1) => {
    try {
      ...
      const res = await axios.get(
        `${apiBase}/api/deleted-students?page=${p}&limit=20`,
        {
      ...
      const msg =
        e.response?.data?.message || "Failed to load deleted students";
      toast.error(msg);
    } finally {
      setLoading(false);
    }
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full p-4">
            <div className="flex justify-between items-center mb-3">
              ...
              <button onClick={() => setSelected(null)} className="px-2 py-1">
                ✕
              </button>
            </div>
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <span className="font-medium">Section:</span> {selected.section}
              </div>
              <div>
              ...
              <div>
                <span className="font-medium">Gender:</span> {selected.gender}
              </div>
              <div>
              ...
              <div>
                <span className="font-medium">Father Mobile:</span>{" "}
                {selected.fatherMobile}
              </div>
              <div>
                <span className="font-medium">Contact:</span> {selected.contact}
              </div>
              <div>
              ...
              <div className="col-span-2">
                ...
                {new Date(selected.deletedAt).toLocaleString()}
              </div>
              <div className="col-span-2">
                <span className="font-medium">Deleted By:</span>{" "}
                {selected.deletedByUsername}
              </div>
            </div>
          </div>
        </div>
