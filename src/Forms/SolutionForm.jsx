...
const ImageDropzone = ({ index, imageUploads, setImageUploads }) => {
...
const SolutionForm = ({ onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Sort solutions by questionNumber before sending to backend
      const sortedSolutions = [...formData.solutions].sort(
        (a, b) => a.questionNumber - b.questionNumber,
      );

      // Prepare solutions with image references
      const validatedSolutions = sortedSolutions.map((sol, index) => {
        const questionImages = [];

        if (imageUploads[index]) {
          questionImages.push(imageReferences[index]);
        }
              ...
              >
                ...
                <div className="mb-4">
                  ...
                  <textarea
                    ...
                    value={formData.solutions[index]?.questionText || ""}
                    onChange={(e) =>
                  ...
                  <div className="text-right text-xs text-gray-500 mt-1">
                    {(formData.solutions[index]?.questionText || "").length}
                    /1000
                  </div>
                </div>
                <ImageDropzone
                        ...
                        <div key={opt} className="flex items-center">
                          <input
                            ...
                            checked={
                              formData.solutions[
                                index
                              ]?.correctOptions?.includes(opt) || false
                            }
