  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const attendanceData = filteredStudents.map(student => ({
        ...
        campus: typeof student.campus === 'object' ? student.campus._id : student.campus,
        subject: filters.subject,
        period: attendanceTime,
        date: attendanceDate.toISOString().split('T')[0],
        present: !attendanceRecords[student.regNumber]?.absent && 
      ...
      // Submit to backend
      await axios.post(`${API_URL}/api/createbulkattendance`, 
        { attendanceData },
        { headers: { Authorization: `Bearer ${token}` } }
      );
    ...
    <div className="bg-white rounded-lg shadow-md p-6">
      ...
      {loading ? (
                ...
                {filteredStudents.map(student => (
                  <tr key={student.regNumber} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      <input
                        type="checkbox"
                        checked={attendanceRecords[student.regNumber]?.absent || false}
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      <input
                        type="checkbox"
                        checked={attendanceRecords[student.regNumber]?.forgiven || false}
                        onChange={() => handleAttendanceChange(student.regNumber, 'forgiven')}
                        className="h-4 w-4 text-orange-600 focus:ring-orange-500 border-gray-300 rounded"
                      />
                    </td>
                  </tr>
        ...
        <div className="mb-6 text-center py-8 bg-gray-50 rounded-lg">
          <p className="text-gray-500">
            {allStudents.length ? 'No students match your filters' : 'No students available'}
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-yellow-50 p-4 rounded-lg border border-yellow-200">
          ...
          <p className="text-2xl font-bold text-yellow-600">{forgivenCount}</p>
      ...
      <div className="flex justify-end space-x-4">
        ...
        <button
          ...
          disabled={loading || !filters.subject || filteredStudents.length === 0}
